...
<div class="card rounded mb-5 mb-xl-8 shadow-lg">
    ...
    <div class="card-header rounded border-0 pt-5">
        ...
        <div class="card-toolbar">
            <a href="{{route('product.create')}}" class="btn btn-sm btn-light-primary">
                        ...
                        <td class="px-3">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="d-flex flex-column">
                                    <a href="{{ route('product.edit', $p->id) }}" class="text-dark fw-bolder text-hover-primary mb-1 fs-6">{{$p->title}}</a>
                                    <span class="text-muted fw-bold text-muted d-block fs-7">{{$p->title_ar}}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-3">
                            <a href="#" class="text-dark fw-bolder text-hover-primary d-block mb-1 fs-6">{{@$p->category->title_en}}</a>
                        </td>
                        <td class="px-3">
                         ...
                         <td class="border text-center">
                            ...
                            <a href="{{ route('product.edit', $p->id) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                            ...
                            <form action="{{ route('product.destroy', $p->id) }}" method="post" id='delform' style="display: inline-block">
                                @csrf
                                @method('delete')
